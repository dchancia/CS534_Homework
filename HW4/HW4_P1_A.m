...
for j=1:length(nodes)
    ...
    % Adaboost.M1
    for i=1:weak_N
        tree = fitctree(X, y, 'MaxNumSplits', nodes(j), 'Weights', w);
...
for i=1:length(nodes)
    t = templateTree('MaxNumSplits',nodes(i));
    mdl = fitcensemble(X, y, 'Method', 'AdaBoostM1', 'Learners',t, ...
        'CrossVal', 'on', 'KFold', 5, 'NumLearningCycles', 300);
    acc = 1 - kfoldLoss(mdl,'Mode','cumulative');
    plot(acc, 'LineWidth', 1.5);
    hold on
    ylim([0.7 1]);
end
legend({"Stumps", "5-node", "10-node"}, 'Location', 'southeast');
